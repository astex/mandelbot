#!/usr/bin/env bash
# SPIKE: end-to-end demo of the two bets — shadow-branch worktree
# snapshots + truncated-JSONL `claude --resume`. Exercises the
# mechanics the mandelbot MCP tools (checkpoint/rewind/fork) use.
#
# Usage: demo/time-travel-demo.sh
#
# Prereqs: `claude` on $PATH, git, python3.
set -euo pipefail
...
echo
echo "=== REWIND worktree to checkpoint-0 + truncate jsonl ==="
git clean -fdxq
...
if echo "$RESUME_OUT" | grep -qi pineapple && ! echo "$RESUME_OUT" | grep -qi banana; then
    echo "  ✓ rewind conversation restore OK (PINEAPPLE only, no BANANA)"
else
    echo "  ✗ rewind FAILED — unexpected output"
    exit 1
fi
