...
/// Core FIFO reader logic. Reads status lines from the FIFO and sends
/// `Message::SetStatus` via the provided callback. Blocks until the FIFO
/// closes or an error occurs.
pub(crate) fn run_fifo_thread(
    tab_id: usize,
    fifo_path: PathBuf,
    ...
    for line in reader.lines() {
        ...
        {
            send(Message::SetStatus(tab_id, s));
        }
    }
}

/// Core tab I/O thread logic. Spawns the PTY, starts a reader thread,
/// runs the event loop, and sends messages via the provided callback.
/// Blocks until the tab shuts down.
pub(crate) fn run_tab_thread(
    ...
    loop {
        match event_rx.recv() {
            Ok(TabEvent::PtyData(bytes)) => {
                ...
                send(Message::TabOutput(id, bg_tasks));
            }
            ...
            Ok(TabEvent::PtyEof) => {
                ...
                send(Message::ShellExited(id, exit_code));
            }
...
pub fn fifo_stream(
    ...
    iced::stream::channel(
        ...
        >| async move {
            ...
            std::thread::spawn(move || {
                run_fifo_thread(tab_id, fifo_path, move |msg| {
