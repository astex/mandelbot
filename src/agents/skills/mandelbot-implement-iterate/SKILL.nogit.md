...
**Warning:** This project is not using git-based isolation. All agents share the same working directory. The generation tab must design task boundaries carefully so children don't conflict on files.

Read `<plugin-dir>/skills/_shared/coord.md` for the shared protocol and `<plugin-dir>/skills/mandelbot-implement-iterate/GENERATION.md` for the generation tab protocol. This file covers the parent's perspective: framing the run, the generation cycle, iteration modes, and exit conditions.
...
**Integration strategy**: `no-git` — all agents write to the same working directory, so no merging is needed between generations. Changes are immediately in place.
...
- **How we work**: include the iteration mode, exit condition, and max_generations. Note that this is a multi-generation iterate run with no VCS isolation — file ownership boundaries are critical. Children of generation tabs should know about the idea convention — the generation tab will relay this.
...
### 6. User checkpoint (if human-in-the-loop)
