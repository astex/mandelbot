...
Write (or reuse) a plan file. Record the iteration mode, integration strategy, exit condition, and max_generations (if applicable) in the plan.
...
- **`blocked:`** — The generation tab is relaying a block from one of its children (or is itself blocked). Read the question, resolve it. Answer in the generation tab's file and ring it. To unblock a grandchild quickly you may instead write a `[DIRECTIVE]` directly into its coord file and ring it using the `**Session:**` header the generation tab recorded there — note that the generation tab won't be notified you did so.
